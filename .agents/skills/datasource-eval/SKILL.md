...
对全部 selector 源 × 每部番，跑全流程解析 + 前 3 条线路 VLC 实播 + 单帧广告启发式。断点续跑安全。
...
每部番约 3–8 分钟（取决于源数与网络）。建议 `run_in_background` 并用日志盯进度。

Ani API 的搜索接口冷启动慢且会限流；MCP 工具内部已重试，脚本对超时也会重启 server。

### 3. 深度采样 (为看图判广告)

对**可用源**（跨番至少成功 1 次）的线路，重新解析并 VLC 长播 28s、在 0/3/8/15/25s 各截一帧。
...
- 枚举 `deep/*/deep.json` 里每个 `(源, 线路)` 的 `frame_00s/08s/15s` 路径。
...
4. `## 全部可用源` — 一行一源，按广告轻→成功率排序，含每部番 ✅/❌ 列 + 广告等级（`*` 表示另有更脏线路）。
5. `## 各源线路拆解` — 每个可用源一个小节，表格**每条线路 × 每部番**（列 = 番剧简称），单元格 ✅/❌/—，外加广告/分辨率/起播/失败原因列。**这一节让主报告自包含，不必点进源页**。
6. `## 🔒 搜索阶段失败` — searchSubjects 就失败的源，表格 源·Tier·原因（验证码 / 域名失效 / 403 等）。
7. `## ❌ 解析/匹配阶段失败` — 搜到但后续步骤失败的源，表格 源·Tier·失败阶段·原因（selector 失配 / js 伪链接 / 视频解析失败 / 播放探测失败等）。
...
- 列级 `-` = 该线路无此项数据（从未成功播放，故无分辨率/起播/广告）。
