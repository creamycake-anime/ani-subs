...
找出快测曾真实播放成功、但深采二次解析未复现截图的 (番剧, 线路)，**复用快测保存的视频 URL** 直接补跑 28s 完整多点截图（省掉二次解析，记录带 `backfilledFromQuick: true` 标记）。补跑失败（URL 过期/截不到帧）则跳过并记日志，该线路保持"未判定"——**不会拿快测的开头两帧充当视觉证据**（那只覆盖 ~4s，判不出轻/中/重）。
